...
const extendedConsonants = 'bcçdðfghjklpqrstvwxz';

const oBrothers = [ "au", "eau"];
const aiBrothers = [ "ai", "é", "è", "ê", "ë"];
const inBrothers = [ "in", "ein", "yn", "ain", "im", "eim", "ym", "aim"];
const anBrothers = [ "an", "en", "am", "em"];
...
export function transliterate(text) {
  ...
  result = result.replace(new RegExp(`(${inBrothers.join('|')})([^${extendedVowels}]+|$)`, 'g'), (match, l1 = "", l2 = "") => diagraphMap['in'] + l2);

  // Replace anBrothers not followed by a vowel with the letter mapped to "an"
  result = result.replace(new RegExp(`(${anBrothers.join('|')})([^${extendedVowels}]+|$)`, 'g'), (match, l1 = "", l2 = "") => diagraphMap['an'] + l2);
